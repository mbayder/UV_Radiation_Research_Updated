...
plt.plot(wavelengths, flux_needed_1, label=f'At {target_age_1} Ga (before GOE)', color='#AA4655', linewidth=5)
plt.plot(wavelengths, flux_needed_2, label=f"At {target_age_2} Ga (after GOE, before NOE)", color='#D5C362', linewidth=5)
plt.plot(wavelengths, flux_needed_3, label=f"At {target_age_3} Ga (after NOE)", color='#329BDB', linewidth=2)
plt.xlabel('Wavelength, nm', fontsize=30)
plt.ylabel(f'Irradiance, {unicodeit.replace("W m^{-2} nm^{-1}")}', fontsize=25)
plt.yscale('log')
...
file_path_save = os.path.join(directory, f"Irradiance_vs_Wavelength_Top_of_Atmosphere_{timestamp}.pdf")
